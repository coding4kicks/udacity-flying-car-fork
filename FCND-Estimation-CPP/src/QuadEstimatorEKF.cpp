...
//  ekfState(6) = ekfState(6) + dtIMU * gyro.z;  // yaw
  ...
  Quaternion<float> dt = Quaternion<float>().FromEuler123_RPY(dtIMU * gyro.x, dtIMU * gyro.y, dtIMU * gyro.z);
  Quaternion<float> q_bar = dt * q_t;
  V3D eulerRPY = q_bar.ToEulerRPY();
  float predictedPitch = eulerRPY.y;
  float predictedRoll = eulerRPY.x;
  ekfState(6) = eulerRPY.z;  // yaw
